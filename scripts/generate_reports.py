...
def append_report_body(body, lab):
    ...
    insert(make_table(["Отметка", "Проверяемое утверждение", "Комментарий"], self_rows, [1000, 5000, 3000], font_size=17))

    insert(heading("10. Краткий вывод", level=1))
    insert(hint_box("[Сформулируйте профессиональный вывод: что установлено, соответствует ли результат требованию, какое решение принято и на каких доказательствах оно основано. Не пересказывайте теорию.]", lines=2))
